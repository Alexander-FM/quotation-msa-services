package com.codesoft.role.controller;
...
import com.codesoft.role.service.RoleServiceImpl;
import com.codesoft.role.utils.RoleConstants;
...
public class RoleController {
  ...
  public ResponseEntity<GenericResponse<List<RoleResponseDto>>> retrieve() {
    final List<RoleResponseDto> employees = this.roleService.findAll();
  ...
  public ResponseEntity<GenericResponse<RoleResponseDto>> retrieveById(@PathVariable(value = "id") final Integer id) {
    final RoleResponseDto employees = this.roleService.findById(id);
    return ResponseEntity.status(HttpStatus.OK)
        .body(GenericResponseUtils.buildGenericResponseSuccess(RoleConstants.FOUND_MESSAGE, employees));
  }

  @PostMapping
  public ResponseEntity<GenericResponse<RoleResponseDto>> create(@RequestBody final RoleRequestDto requestDto) {
  ...
  public ResponseEntity<GenericResponse<RoleResponseDto>> update(@PathVariable(value = "id") final Integer id,
      @RequestBody final RoleRequestDto requestDto) {
    if (id == null || id <= 0) {
